...
#sys.stdout = open('out.txt', 'w') ## ADDED!
...
if not args.train_embeddings:
    emb_path = args.emb_path #directory containing word embeddings
    vect_path = os.path.join(args.data_path.split('/')[0], 'embeddings.pkl') #directory containing data
    vectors = {}
    with open(emb_path, 'rb') as f:
        for l in f:
            ...
            if word in vocab:
                ...
                vectors[word] = vect # set dictionary which assigns each word a word vector
    embeddings = np.zeros((vocab_size, args.emb_size))
    words_found = 0
    for i, word in enumerate(vocab):
        try:
            embeddings[i] = vectors[word] #set embeddings matrix (assign word embeddings to vocab)
            words_found += 1
        except KeyError:
            embeddings[i] = np.random.normal(scale=0.6, size=(args.emb_size, )) #else: initiate random
    embeddings = torch.from_numpy(embeddings).to(device) # save vocab embeddings
...
def train(epoch):
    ...
    cur_kl_theta = round(acc_kl_theta_loss / cnt, 2)
    cur_real_loss = round(cur_loss + cur_kl_theta, 2)
    print('Timestamp: ', datetime.datetime.now().strftime("%A, %d. %B %Y %I:%M%p")) # ADDED timestamp
    print('*'*100)
...
def evaluate(m, source, tc=False, td=False):
    ...
    with torch.no_grad():
        ...
        ### do dc and tc here (document completion / topic coherence?)
        acc_loss = 0
        cnt = 0
        ...
        return ppl_dc, beta, theta, preds
...
if args.mode == 'train':
    ...
    for epoch in range(1, args.epochs):
        train(epoch)
        val_ppl, my_beta, my_theta, my_preds = evaluate(model, 'val')
        print("This is the evaluation of epoch", epoch, ": ", val_ppl) # inserting this to test the training
        if val_ppl < best_val_ppl:
            with open(ckpt, 'wb') as f:
                torch.save(model, f)
            best_epoch = epoch
            best_val_ppl = val_ppl
            visualize(model) ## ADDED
        else:
    ...
    print('Visualizing model quality after training...') ## ADDED
    visualize(model) ## ADDED
    print('Maximum best epoch: ', best_epoch)
    print('Maximum best validation score: ', best_val_ppl)
    print('List of all validation scores: ', all_val_ppls)
    with open(ckpt, 'rb') as f:
        model = torch.load(f)
    model = model.to(device)
    val_ppl, my_beta, my_theta, my_preds = evaluate(model, 'val')
    #
    #### EXTRACTING beta, theta, preds and rho
    #
    filename_beta = 'betas-{}-{}.pt'.format(args.num_topics, args.dataset)
    torch.save(my_beta,filename_beta)
    #
    filename_theta = 'theta-{}-{}.pt'.format(args.num_topics, args.dataset)
    torch.save(my_theta,filename_theta)
    #
    filename_preds = 'preds-{}-{}.pt'.format(args.num_topics, args.dataset)
    torch.save(my_preds,filename_preds)
    #
    filename_rho = 'rho-{}-{}.pt'.format(args.num_topics, args.dataset)
    try:
        rho_etm = model.rho.weight.cpu()
    except:
        rho_etm = model.rho.cpu()
    torch.save(rho_etm,filename_rho)
    ####
    #
    ...
    with open(ckpt, 'rb') as f:
        model = torch.load(f, map_location=torch.device('cpu')) # added cpu location here
    model = model.to(device)
    model.eval()

    with torch.no_grad():
        ## get document completion perplexities
        test_ppl, my_beta, my_theta, my_preds = evaluate(model, 'test', tc=args.tc, td=args.td)
        ...
        ## show topics
        beta = model.get_beta()
        print(beta) # added this to check model
        ...
        if args.train_embeddings:
            ...
            try:
                ...
                rho_etm = model.rho.cpu()
            print("This is my Rho ETM-Matrix: \n") # added this to check model
